...
Write-Host "Ensuring $Visibility repository exists: $RepoFullName"
$repoExists = $true
try {
...
if (-not $repoExists) {
  ...
  Run-Gh repo create $RepoFullName $visibilityFlag --source . --remote origin --push --description "Smeta-Stroyka AI Calculator: local AI tool for construction estimates, plans, proposals and price lists"
} else {
...
function New-Issue {
  ...
  if ($Close) {
    Run-Gh issue close $number --repo $RepoFullName --comment "Recorded in the initial project commits." | Out-Null
  }
